...
export function UserKeysCard() {
  ...
  const load = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Tải danh sách khóa người dùng thất bại");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleToggle = async (item: UserKey) => {
    ...
    try {
      ...
      setItems(data.items);
      toast.success(item.enabled ? "Đã vô hiệu khóa người dùng" : "Đã bật khóa người dùng");
    } catch (error) {
  ...
  const handleCopy = async (value: string) => {
    try {
      ...
      toast.error("Sao chép thất bại, hãy sao chép thủ công");
    }
  };
        ...
        <CardContent className="space-y-6 p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-stone-500">Tạo khóa riêng cho người dùng thường; người dùng thường chỉ vào được trang vẽ ảnh, không xem được cài đặt và pool tài khoản.</p>
              </div>
            </div>
          ...
          {revealedKey ? (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm text-emerald-900">
              <div className="font-medium">Khóa mới chỉ hiển thị một lần, hãy lưu ngay:</div>
          ...
          {isLoading ? (
            ...
            <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
              Chưa có khóa người dùng thường. Bấm nút góc trên phải để tạo và phân phát cho người khác.
            </div>
          ) : (
                    ...
                    <div className="min-w-0 space-y-2">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge variant={item.enabled ? "success" : "secondary"} className="rounded-md">
                          {item.enabled ? "Đã bật" : "Đã vô hiệu"}
                        </Badge>
                      </div>
        ...
        <DialogContent className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              Có thể điền tên ghi chú để phân biệt người dùng; sau khi tạo sẽ sinh một khóa gốc chỉ xem được một lần.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Ví dụ: Bạn thiết kế A, tài khoản tạm vận hành"
      ...
      <Dialog open={Boolean(deletingItem)} onOpenChange={(open) => (!open ? setDeletingItem(null) : null)}>
        <DialogContent className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              Xác nhận xóa khóa người dùng "{deletingItem?.name}"? Sau khi xóa, khóa này sẽ không thể gọi API nữa.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
      ...
      >
        <DialogContent className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              Có thể sửa tên ghi chú; nếu cần đổi khóa riêng, điền khóa gốc mới. Để trống thì giữ nguyên khóa hiện tại.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs leading-5 text-stone-500">
                Sau khi lưu, khóa cũ mất hiệu lực ngay, khóa mới có hiệu lực. Hệ thống chỉ lưu hash, không hiển thị khóa hiện tại.
              </p>
            </div>
          </div>
          <DialogFooter>
