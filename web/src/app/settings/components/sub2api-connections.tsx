...
export function Sub2APIConnections() {
    ...
    pollTimerRef.current = window.setInterval(() => {
      void fetchSub2APIServers()
        ...
        .catch((error) => {
          ...
          toast.error(error instanceof Error ? error.message : "Truy vấn tiến trình nhập thất bại");
        });
    }, 1500);
  ...
  const handleFetchGroups = async () => {
    if (!editingServer) {
      toast.error("Vui lòng lưu kết nối trước khi lấy nhóm");
      return;
    }
    ...
    try {
      ...
      if (data.groups.length === 0) {
        toast.message("Máy chủ từ xa chưa cấu hình nhóm");
      } else {
        toast.success(`Đã đọc được ${data.groups.length} nhóm`);
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Lấy nhóm thất bại");
    } finally {
  ...
  const handleBrowseAccounts = async (server: Sub2APIServer) => {
    ...
    try {
      ...
      setAccountPage(1);
      setBrowserOpen(true);
      toast.success(`Đọc thành công, tổng ${accounts.length} tài khoản OpenAI`);
    } catch (error) {
  ...
  const handleStartImport = async () => {
    ...
    if (selectedIds.length === 0) {
      toast.error("Vui lòng chọn tài khoản cần nhập");
      return;
    }
        ...
        <CardContent className="space-y-6 p-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-stone-500">
                  Sau khi cấu hình máy chủ Sub2API, có thể truy vấn tài khoản OpenAI OAuth trong đó và nhập hàng loạt vào pool cục bộ.
                </p>
              </div>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-3 rounded-xl bg-stone-50 px-6 py-10 text-center">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-stone-400">Bấm "Thêm kết nối" để lưu thông tin Sub2API của bạn.</p>
              </div>
            </div>
          ) : (
                          ...
                          return (
                            <div className="rounded-lg border border-stone-200 bg-white px-3 py-3">
                              ...
                              <div className="mt-2 flex flex-wrap gap-2 text-xs text-stone-500">
                                <span>Thêm {importJob.added}</span>
          ...
          <div className="rounded-xl bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-500">
            ...
            <ul className="mt-1 list-inside list-disc space-y-0.5">
              <li>Nhập địa chỉ Sub2API và tài khoản quản trị (hoặc Admin API Key), lưu thành một kết nối.</li>
              <li>Bấm "Đồng bộ" ở một kết nối sẽ lấy danh sách tài khoản platform=openai và type=oauth trong đó.</li>
              <li>Sau khi tích chọn tài khoản cần, backend sẽ lấy access_token đồng thời, tự nhập vào pool cục bộ và làm mới trạng thái.</li>
              <li>Chỉ đọc access_token trong credential sub2api; các trường như refresh_token không được ghi cục bộ.</li>
            </ul>
          </div>
        </CardContent>
      </Card>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            <DialogTitle>{editingServer ? "Sửa kết nối" : "Thêm kết nối"}</DialogTitle>
            <DialogDescription className="text-sm leading-6">
              {editingServer ? "Sửa thông tin kết nối Sub2API" : "Thêm một kết nối Sub2API mới"}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ví dụ: sub2api tự dựng"
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      placeholder={editingServer ? "Để trống thì không đổi mật khẩu" : "Mật khẩu quản trị"}
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    placeholder={editingServer ? "Để trống thì không đổi khóa" : "Sub2API Admin API Key"}
                    className="h-11 rounded-xl border-stone-200 bg-white pr-10"
                  />
                  <button
                    type="button"
            ...
            <div className="space-y-2">
              ...
              {remoteGroups && remoteGroups.length > 0 ? (
                <Select value={formGroupId || "__all__"} onValueChange={(value) => setFormGroupId(value === "__all__" ? "" : value)}>
                  ...
                  <SelectContent>
                    <SelectItem value="__all__">Tất cả nhóm (không giới hạn)</SelectItem>
                ...
                <Input
                  ...
                  placeholder="Để trống để đồng bộ tất cả nhóm; hoặc điền ID nhóm / ungrouped"
                  className="h-11 rounded-xl border-stone-200 bg-white"
                />
              )}
              {editingServer ? (
                <div className="flex items-center justify-between gap-2 text-xs text-stone-500">
                  ...
                  >
                    ...
                    {remoteGroups ? "Lấy lại" : "Lấy nhóm"}
                  </Button>
                </div>
              ) : (
                <div className="text-xs text-stone-500">
                  Sau khi thêm kết nối, có thể bấm「Lấy nhóm」trong hộp thoại chỉnh sửa để chọn nhóm cụ thể.
                </div>
              )}
            </div>
          </div>
      ...
      <Dialog open={browserOpen} onOpenChange={setBrowserOpen}>
        <DialogContent showCloseButton={false} className="max-h-[90vh] max-w-5xl rounded-2xl p-6">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="relative min-w-[260px]">
              ...
              <Input
                ...
                placeholder="Tìm email, gói hoặc tên"
          ...
          <div className="rounded-xl border border-stone-200">
            <div className="flex items-center justify-between border-b border-stone-100 px-4 py-3 text-sm text-stone-500">
              <div className="flex items-center gap-3">
                ...
                />
                <span>Kết quả lọc {filteredAccounts.length}</span>
              </div>
              <span>Đã chọn {selectedIds.length}</span>
            </div>
            <div className="max-h-[420px] overflow-auto">
              {pagedAccounts.length === 0 ? (
                <div className="flex items-center justify-center py-12 text-sm text-stone-400">Không có tài khoản khớp</div>
              ) : (
          ...
          <div className="flex items-center justify-between text-sm text-stone-500">
            <span>
              Từ {filteredAccounts.length === 0 ? 0 : (safeAccountPage - 1) * currentPageSize + 1} -{" "}
