...
const includeLabels: Array<{ key: keyof BackupInclude; label: string }> = [
  ...
  { key: "logs", label: "Nhật ký điều phối và gọi" },
  { key: "image_tasks", label: "Bản ghi tác vụ ảnh" },
  { key: "accounts_snapshot", label: "Ảnh chụp tài khoản" },
  { key: "auth_keys_snapshot", label: "Ảnh chụp khóa người dùng" },
  { key: "images", label: "Thư mục tệp ảnh" },
];
...
export function BackupSettingsCard() {
  ...
  const handleOpenDetail = async (key: string) => {
    ...
    try {
      ...
      setDetail(null);
      toast.error(error instanceof Error ? error.message : "Đọc chi tiết bản sao lưu thất bại");
    } finally {
  ...
  const handleDownload = async (key: string, name: string) => {
    try {
      ...
      if (!authKey) {
        toast.error("Phiên đăng nhập đã hết hạn, hãy đăng nhập lại rồi tải xuống");
        return;
      }
      ...
      if (!response.ok) {
        ...
        try {
          ...
          message = response.status === 401 ? "Đăng nhập đã hết hạn, hãy đăng nhập lại rồi thử" : message;
        }
      ...
      toast.success("Đã bắt đầu tải bản sao lưu");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Tải bản sao lưu thất bại");
    }
  };
    ...
    <>
      <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
        <CardContent className="space-y-6 p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-stone-500">Sao lưu định kỳ dữ liệu quan trọng lên Cloudflare R2, hỗ trợ mã hóa tùy chọn, luân chuyển, chạy thủ công và dọn lịch sử.</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Badge variant={backupState?.running ? "warning" : backupState?.last_status === "success" ? "success" : "secondary"} className="rounded-md">
                {backupState?.running ? "Đang sao lưu" : backupState?.last_status === "success" ? "Gần đây thành công" : backupState?.last_status === "error" ? "Gần đây thất bại" : "Chưa chạy"}
              </Badge>
            </div>
          </div>

          <div className="rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 text-sm leading-6 text-stone-600">
            Tài khoản và khóa người dùng được xuất snapshot logic từ backend lưu trữ hiện tại, không phụ thuộc lớp dưới là `json`, `sqlite`, `postgres` hay `git`. Thư mục ảnh mặc định không sao lưu, tránh dung lượng quá lớn.
          </div>
          ...
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Khoảng sao lưu định kỳ</label>
            ...
            <p className="text-xs text-stone-500">Đơn vị phút, sau khi khởi động dịch vụ sẽ tự động chạy theo khoảng này.</p>
          </div>

          <div className="space-y-2">
            <label className="text-sm text-stone-700">Số bản sao lưu giữ lại</label>
            ...
            <p className="text-xs text-stone-500">Sau khi tải lên thành công sẽ tự động xóa các bản sao lưu cũ hơn. Điền `0` nghĩa là không tự động luân chuyển.</p>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-stone-700">Mật khẩu mã hóa</label>
            <Input type="password" value={String(backup.passphrase || "")} onChange={(event) => setBackupField("passphrase", event.target.value)} placeholder={backup.encrypt ? "Bắt buộc khi bật mã hóa" : "Để trống"} className="h-10 rounded-xl border-stone-200 bg-white" />
            <p className="text-xs text-stone-500">Chỉ dùng khi bật mã hóa. Hãy giữ kỹ, nếu không sẽ không giải mã được nội dung sao lưu.</p>
          </div>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-stone-500">Chọn theo thành phần dữ liệu cần đưa vào gói sao lưu.</p>
          </div>
          ...
          <div>
            <div className="text-xs text-stone-500">Hoàn tất gần nhất</div>
          ...
          {isLoadingBackups ? (
            ...
            <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
              Chưa có bản ghi sao lưu từ xa. Sau khi lưu cấu hình và chạy sao lưu thủ công một lần sẽ hiện ở đây.
            </div>
          ) : (
            <div className="space-y-3">
              {backups.map((item) => {
                  ...
                  <div key={item.key} className="flex flex-col gap-3 rounded-xl border border-stone-200 bg-white px-4 py-4 md:flex-row md:items-center md:justify-between">
                    <div className="min-w-0 space-y-2">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-stone-500">
                        ...
                        <span>Cập nhật {formatDateTime(item.updated_at)}</span>
        ...
        <DialogContent className="flex max-h-[85vh] max-w-3xl flex-col overflow-hidden rounded-2xl border-white/80 bg-white">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
            {detailLoading ? (
              ...
              <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">
                Tạm thời không đọc được chi tiết sao lưu; nếu đây là bản sao lưu mã hóa, hãy xác nhận đã nhập đúng mật khẩu mã hóa và lưu cấu hình trước.
              </div>
            ) : (
              <>
                <div className="grid gap-3 rounded-xl border border-stone-200 bg-stone-50 px-4 py-4 text-sm text-stone-600 md:grid-cols-2">
                  ...
                  <div>
                    <div className="text-xs text-stone-500">Cách kích hoạt</div>
                ...
                <div className="space-y-3">
                  <h4 className="text-sm font-medium text-stone-800">Nội dung ảnh chụp</h4>
