...
// Nhãn hiển thị cho từng trạng thái tài khoản (giá trị backend giữ nguyên tiếng Trung).
...
const accountStatusOptions: { label: string; value: AccountStatus | "all" }[] = [
  ...
  { label: statusLabels["正常"], value: "正常" },
  { label: statusLabels["限流"], value: "限流" },
  { label: statusLabels["异常"], value: "异常" },
  { label: statusLabels["禁用"], value: "禁用" },
];
...
const metricCards = [
  ...
  { key: "limited", label: "Tài khoản bị giới hạn", color: "text-orange-500", icon: CircleAlert },
  { key: "abnormal", label: "Tài khoản bất thường", color: "text-rose-500", icon: CircleOff },
  { key: "disabled", label: "Tài khoản vô hiệu", color: "text-stone-500", icon: Ban },
...
function formatRestoreAt(value?: string | null) {
  ...
  const relative = diffMs > 0 ? `Còn ${days}d ${hours}h` : "Đã đến giờ khôi phục";
...
function AccountsPageContent() {
  ...
  const loadModels = async () => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Tải danh sách model thất bại";
      toast.error(message);
    } finally {
    ...
    try {
        ...
        const pollTimer = setInterval(async () => {
          try {
            ...
            if (p.done) {
              ...
              if (!p.result) {
                reject(new Error("Kết quả làm mới rỗng"));
                return;
              }
              // 更新最终进度显示
              setProgress((prev) => ({
      ...
      if (relogined > 0) {
        setProgress({
          ...
          current: 0,
          total: relogined,
          message: `Đang thử gỡ trạng thái bất thường cho ${relogined} tài khoản`,
          email: "",
        });
        // 模拟重新登录进度
        let reCount = 0;
          ...
          const timer = setInterval(() => {
            ...
            if (reCount >= relogined) {
              ...
              setProgress({
                ...
                current: relogined,
                total: relogined,
                message: "Gỡ trạng thái bất thường hoàn tất",
                email: "",
              });
      ...
      if ((data.errors ?? []).length > 0) {
        ...
        toast.success(`Làm mới thành công ${data.refreshed} tài khoản${relogined > 0 ? `, đã kích hoạt đăng nhập lại ${relogined} tài khoản` : ""}`);
      }
    } catch (error) {
  ...
  const handleReLogin = async (accessTokens: string[]) => {
    ...
    if (abnormalTokens.length === 0) {
      toast.error("Không có tài khoản bất thường nào trong số đã chọn");
      return;
    }

    if (abnormalTokens.length < accessTokens.length) {
      toast.info(`Đã lọc bỏ ${accessTokens.length - abnormalTokens.length} tài khoản không bất thường`);
    }
    ...
    setProgress({ visible: true, current: 0, total, message: "Đang thử khôi phục tài khoản bất thường...", email: "" });
    ...
    try {
        ...
        const pollTimer = setInterval(async () => {
          try {
            ...
            if (p.done) {
              ...
              setProgress((prev) => ({ ...prev, current: prev.total, message: "Quy trình khôi phục đã hoàn tất" }));
              setRefreshSummary(null);
              resolve();
            } else {
              // 实时更新进度
              ...
              setProgress((prev) => ({
                ...
                current: p.processed,
                email: emailHint,
                message: "Đang thử khôi phục tài khoản bất thường...",
              }));
      ...
      toast.success(`Quy trình khôi phục đã hoàn tất toàn bộ`);
    } catch (error) {
      setProgress({ visible: false, current: 0, total: 0, message: "", email: "" });
      setRefreshSummary(null);
      const message = error instanceof Error ? error.message : "Đăng nhập lại thất bại";
      toast.error(message);
    } finally {
    ...
    <>
      <section className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl font-semibold tracking-tight">Quản lý Account Pool</h1>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            Làm mới toàn bộ thông tin và hạn mức
          </Button>
          <AccountImportDialog
            disabled={isLoading || isRefreshing || isDeleting}
            onImported={(items) => {
        ...
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            ...
            <DialogDescription className="text-sm leading-6">
              Thay đổi thủ công trạng thái tài khoản và proxy riêng.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                <Input
                  ...
                  placeholder="Để trống dùng proxy toàn cục, ví dụ http://127.0.0.1:7890"
          ...
          <CardContent className="p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {availableModels.length > 0 ? (
                ...
                <span className="text-sm text-stone-400">Đang tải danh sách model...</span>
              ) : (
                <span className="text-sm text-stone-400">Hiện chưa có model khả dụng</span>
              )}
            </div>
          </CardContent>
        </Card>
          ...
          <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
            <CardContent className="flex flex-col items-center justify-center gap-3 px-6 py-14 text-center">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-stone-500">Đang đồng bộ danh sách và trạng thái tài khoản từ backend.</p>
              </div>
            </CardContent>
          </Card>
        ) : null}
          ...
          <CardContent className="space-y-0 p-0">
            <div className="flex flex-col gap-3 border-b border-stone-100 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex flex-wrap items-center gap-2 text-sm text-stone-500">
                ...
                >
                  ...
                  Làm mới thông tin & hạn mức đã chọn
                </Button>
                <Button
                  variant="ghost"
                  ...
                  title="Thử đăng nhập lại bằng mật khẩu để khôi phục tài khoản"
                >
                  {isRelogining ? <LoaderCircle className="size-4 animate-spin" /> : <LogIn className="size-4" />}
                  Thử khôi phục tài khoản bất thường
                </Button>
                <Button
                  variant="ghost"
              ...
              <table className="w-full min-w-[1000px] text-left">
                <thead className="border-b border-stone-100 text-[11px] text-stone-400 uppercase tracking-[0.18em]">
                  <tr>
                    ...
                    <th className="w-32 px-4 py-3">Thời gian tạo</th>
                    <th className="w-24 px-4 py-3">Hạn mức</th>
                    <th className="w-40 px-4 py-3">Thời gian khôi phục</th>
                    <th className="w-18 px-4 py-3">Đang xử lý</th>
                          ...
                          {(() => {
                            ...
                            return (
                              <span
                                ...
                                title={
                                  inflight > 0
                                    ? "Số ảnh đang được tạo. Khi bể tài khoản rảnh mà giá trị này vẫn > 0, nghĩa là slot đồng thời bị rò rỉ và tài khoản đã bị âm thầm loại khỏi lịch điều phối"
                                    : "Hiện không có tác vụ tạo ảnh nào đang chạy"
                                }
                              >
                                {inflight}
                              </span>
              ...
              {!isLoading && currentRows.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-3 px-6 py-14 text-center">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-stone-500">Điều chỉnh bộ lọc hoặc từ khóa tìm kiếm rồi thử lại.</p>
                  </div>
                </div>
              ) : null}
            </div>
            ...
            <div className="border-t border-stone-100 px-4 py-4">
              <div className="flex items-center justify-center gap-3 overflow-x-auto whitespace-nowrap">
                ...
                {Math.min(startIndex + Number(pageSize), filteredAccounts.length)} trong tổng{" "}
                {filteredAccounts.length}
                </div>
