...
export function ImageComposer({
  ...
  const qualityLabel = qualityOptions.find((option) => option.value === imageQuality)?.label || "Tự động";
  const ratioLabel = imageRatio === "auto" ? "auto" : `${imageRatio}(${imageTier})`;
  const imageSizeLabel = `${qualityLabel} · ${ratioLabel} · ${imageCount || 1} ảnh`;
            ...
            {referenceImages.map((image, index) => (
              <div key={`${image.name}-${index}`} className="relative size-14 shrink-0 sm:size-16">
                ...
                <button
                  ...
                  aria-label={`Bỏ ảnh tham chiếu ${image.name || index + 1}`}
                >
                  <X className="size-3" />
                </button>
              </div>
            ...
            <Textarea
              ...
              placeholder={
                referenceImages.length > 0
                  ...
                  : "Nhập nội dung hình ảnh bạn muốn tạo, hoặc dán ảnh trực tiếp"
              }
              onKeyDown={(event) => {
            ...
            {isDraggingImage ? (
              <div className="pointer-events-none absolute inset-0 z-20 flex items-center justify-center rounded-[24px] border-2 border-dashed border-stone-900 bg-white/85 text-sm font-medium text-stone-900 backdrop-blur-[1px] sm:rounded-[32px]">
                <div className="flex items-center gap-2 rounded-full bg-stone-950 px-4 py-2 text-white shadow-lg">
                  ...
                  <span>Thả để tải lên ảnh tham chiếu</span>
                </div>
              </div>
            ) : null}
              ...
              <div className="flex items-end justify-between gap-2 sm:gap-3">
                <div className="hide-scrollbar flex min-w-0 flex-1 flex-nowrap items-center gap-1.5 overflow-x-auto pb-0.5 sm:flex-wrap sm:gap-3 sm:overflow-visible sm:pb-0">
                  <Button
                    ...
                    aria-label={referenceImages.length > 0 ? "Thêm ảnh tham chiếu" : "Tải lên"}
                  >
                  ...
                  <div className="relative flex h-9 min-w-0 shrink items-center rounded-full bg-transparent text-[11px] sm:h-auto sm:shrink-0 sm:text-[13px]">
                    ...
                    {isSizeMenuOpen ? (
                      ...
                      >
                        <h3 className="mb-3 text-base font-semibold text-stone-950">Cài đặt hình ảnh</h3>
                        <div className="mb-3">
                          <div className="mb-2 text-sm font-medium text-stone-900">Model</div>
                          <Select
                            value={imageModel}
                            onValueChange={(value) => {
                        ...
                        <div className="mb-3">
                          <div className="mb-2 text-sm font-medium text-stone-900">Chất lượng</div>
                        ...
                        <div className="mb-3">
                          <div className="mb-2 flex items-center gap-1.5 text-sm font-medium text-stone-900">
                            Kích thước <Info className="size-3.5 text-stone-400" />
                          </div>
                        ...
                        <div className="mb-3">
                          <div className="mb-2 flex items-center gap-1.5 text-sm font-medium text-stone-900">
                            Tỉ lệ khung <Info className="size-3.5 text-stone-400" />
                          </div>
