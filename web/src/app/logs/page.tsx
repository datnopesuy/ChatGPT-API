...
function LogsContent() {
  ...
  const loadLogs = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Tải nhật ký thất bại");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success(`Đã xóa ${data.removed} bản ghi nhật ký`);
      setDeletingItems([]);
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Select value={type} onValueChange={setType}>
            ...
            <SelectContent>
              <SelectItem value={LogType.Call}>Nhật ký gọi API</SelectItem>
        ...
        <CardContent className="p-0">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-100 px-5 py-4">
            <div className="flex flex-wrap items-center gap-3 text-sm text-stone-600">
              <span>Tổng {items.length} bản ghi</span>
              ...
              {selectedIds.length > 0 ? <span>Đã chọn {selectedIds.length} bản ghi</span> : null}
            </div>
            ...
            <Table className="min-w-[900px]">
              <TableHeader>
                <TableRow>
                  ...
                  {isCallLog ? <TableHead>Thời gian xử lý</TableHead> : null}
          ...
          <div className="flex items-center justify-end gap-2 border-t border-stone-100 px-4 py-3 text-sm text-stone-500">
            <span>Trang {safePage} / {pageCount}, tổng {items.length} bản ghi</span>
      ...
      <Dialog open={deletingItems.length > 0} onOpenChange={(open) => (!open ? setDeletingItems([]) : null)}>
        <DialogContent showCloseButton={false} className="rounded-2xl p-6">
          <DialogHeader className="gap-2">
            <DialogTitle>{deletingItems.length === 1 ? "Xóa nhật ký" : "Xóa nhật ký đã chọn"}</DialogTitle>
            <DialogDescription className="text-sm leading-6">
              Xác nhận xóa {deletingItems.length} bản ghi nhật ký? Không thể khôi phục sau khi xóa.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
