...
const statusText = (status: string) => ({ queued: "Đang chờ", running: "Đang tạo", success: "Hoàn tất", error: "Thất bại" }[status] || status);
...
export function EditableFilePanel({ title, kind, endpoint, defaultPrompt, imageRequired }: Props) {
  ...
  const startRename = (id: string) => {
    setRenamingId(id);
    setRenamingTitle(drafts[id]?.title || titleOfPrompt(drafts[id]?.prompt || "", `Tác vụ ${kind.toUpperCase()}`));
  };
  const commitRename = () => {
  ...
  return (
    ...
    <div className="grid h-full min-h-[calc(100vh-148px)] overflow-hidden rounded-lg border border-stone-200 bg-white shadow-sm dark:border-white/10 dark:bg-stone-950 lg:grid-cols-[288px_420px_minmax(0,1fr)]">
      <aside className="flex min-h-0 flex-col border-b border-stone-200 bg-stone-50/70 dark:border-white/10 dark:bg-white/[0.02] lg:border-r lg:border-b-0">
        ...
        <div className="min-h-0 flex-1 overflow-auto p-2">
          {visibleTasks.length ? visibleTasks.map((task) => {
            ...
            <div className="flex h-full items-center justify-center text-sm text-stone-400 dark:text-stone-500">Chưa có bản ghi</div>
          )}
        </div>
      </aside>
      ...
      <section className="flex min-h-0 flex-col border-b border-stone-200 dark:border-white/10 lg:border-r lg:border-b-0">
        ...
        <div className="min-h-0 flex-1 space-y-5 overflow-auto p-5">
          <div className="space-y-2">
            <Label htmlFor={`${endpoint}-prompt`} className="text-xs font-semibold text-stone-700 dark:text-stone-300">Yêu cầu</Label>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <Label className="text-xs font-semibold text-stone-700 dark:text-stone-300">Ảnh tham chiếu</Label>
      ...
      <section className="flex min-h-0 flex-col">
        <div className="flex h-14 items-center justify-between border-b border-stone-200 px-5 dark:border-white/10">
          <h2 className="text-sm font-semibold text-stone-950 dark:text-stone-50">Trạng thái tạo</h2>
          ...
          {selectedTask ? (
            <div className="space-y-5">
              <div className="grid gap-3 sm:grid-cols-[150px_150px_minmax(0,1fr)]">
                ...
                <div className="rounded-md border border-stone-200 bg-stone-50 p-4 dark:border-white/10 dark:bg-white/[0.03]">
                  <div className="text-xs text-stone-500 dark:text-stone-400">Đã chạy</div>
              ...
              {selectedTask.result ? (
                <div className="space-y-3 rounded-md border border-stone-200 bg-white p-4 dark:border-white/10 dark:bg-white/[0.03]">
                  <div className="text-sm font-semibold text-stone-950 dark:text-stone-50">Kết quả tạo</div>
                  <ResultFile href={selectedTask.result.primary_url} icon={<FileText className="size-4" />} label={kind === "ppt" ? "Tệp PPT" : "Tệp PSD"} />
                  <ResultFile href={selectedTask.result.zip_url} icon={<FileArchive className="size-4" />} label="Gói tài nguyên" />
                </div>
              ) : null}
            ...
            <div className="flex h-full min-h-80 items-center justify-center text-sm text-stone-400 dark:text-stone-500">Chưa có tác vụ</div>
          )}
        </div>
      </section>
    </div>
    <Dialog open={!!deleteConfirm} onOpenChange={(open) => !open && setDeleteConfirm(null)}>
      <DialogContent className="rounded-xl" showCloseButton={false}>
        <DialogHeader>
          <DialogTitle>{deleteConfirm?.type === "all" ? "Xóa toàn bộ lịch sử" : "Xóa bản ghi"}</DialogTitle>
          <DialogDescription>
            {deleteConfirm?.type === "all" ? "Xác nhận xóa toàn bộ lịch sử loại này?" : "Xác nhận xóa bản ghi này?"}
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
