...
const latestVersionUrl =
  "https://raw.githubusercontent.com/datnopesuy/ChatGPT-API/main/VERSION";
const latestChangelogUrl =
  "https://raw.githubusercontent.com/datnopesuy/ChatGPT-API/main/CHANGELOG.md";
  ...
  const checkLatestRelease = useCallback(
    async (showMessage = false) => {
      ...
      try {
        ...
        if (showMessage) toast.success("Đã lấy thông tin phiên bản mới nhất");
      } catch {
        setLatestVersion(currentVersion);
        setReleases(localReleases);
        if (showMessage) toast.error("Không lấy được thông tin phiên bản mới nhất");
      } finally {
        setChecking(false);
      }
    },
